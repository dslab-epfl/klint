...
while ! NF=../nf/bridge OS=dpdk NET=dpdk BATCH_SIZE=32 ./bench.sh '../env' $bridge_args ; do sleep 5; done
mv 'results' "$exp_dir/results/bridge-dpdk"
while ! NF=../nf/bridge NF_EXT=.o CC=musl-gcc OS=linux NET=tinynf ./bench.sh '../env' $bridge_args ; do sleep 5; done
...
for nf in bridge firewall maglev nat policer; do
  ...
  while ! NF_EXT=.o CC=musl-gcc NF=../nf/$nf OS=linux NET=tinynf ./bench.sh '../env' $extra_arg $singledir_args $layer ; do
    sleep 5
  done
