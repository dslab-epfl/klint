...
;   3. the _start() function is invoked and all of the following conditions are satisfied:
...
;       - The stack is 16-byte aligned before _start() is invoked
;
;   We assume that:
...
.64_bit:
    ...
    ; boundary before _start() is invoked. We instruct the assembler to align
    ; stack_bottom to a 16-byte boundary.
    mov esp, stack_bottom

    ; At this point all the preconditions have been satisfied and NFOS can invoke _start
    call _start

    ; _start never returns, therefore this code is unreachable.
    hlt
...
; We guarantee that the stack pointer is aligned to 16 bytes before _start() is
