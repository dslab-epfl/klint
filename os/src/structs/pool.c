...
void dchain_impl_init(struct dchain_cell* cells, size_t size)
/*@ requires 0 < size &*&
             size < INT_MAX - 2 &*&
...
bool dchain_impl_allocate_new_index(struct dchain_cell* cells, size_t* index)
...
bool dchain_impl_free_index(struct dchain_cell* cells, size_t index)
...
int dchain_impl_get_oldest_index(struct dchain_cell* cells, size_t* index)
/*@ requires dchainip(?dc, cells) &*& *index |-> ?i; @*/
...
int dchain_impl_rejuvenate_index(struct dchain_cell* cells, size_t index)
/*@ requires dchainip(?dc, cells) &*&
             0 <= index &*& index < dchaini_irange_fp(dc); @*/
             /*@ ensures (dchaini_allocated_fp(dc, index) ?
...
bool dchain_impl_is_index_allocated(struct dchain_cell* cells, size_t index)
/*@ requires dchainip(?dc, cells) &*&
             0 <= index &*& index < dchaini_irange_fp(dc); @*/
             /*@ ensures dchainip(dc, cells) &*&
                         dchaini_allocated_fp(dc, index) ? result == 1 : result == 0; @*/
{
    ...
    //@ dcells_limits(cells);
    //@ extract_heads(cells, cls);
    size_t lifted = index + INDEX_SHIFT;
    //@ extract_cell(cells, cls, lifted);
    struct dchain_cell* liftedp = cells + lifted;
    int lifted_next = liftedp->next;
    int lifted_prev = liftedp->prev;
...
struct os_pool* os_pool_alloc(size_t size)
  ...
  /*@ ensures result == 0 ?
   ...
   @*/
{
  struct os_pool* chain_alloc = (struct os_pool*) os_memory_alloc(1, sizeof(struct os_pool));
  ...
  /*@
    ...
    @*/
  struct dchain_cell* cells_alloc = (struct dchain_cell*) os_memory_alloc((size + DCHAIN_RESERVED), sizeof (struct dchain_cell));
  chain_alloc->cells = cells_alloc;

  time_t* timestamps_alloc = (time_t*) os_memory_alloc(size, sizeof(time_t));
  chain_alloc->timestamps = timestamps_alloc;

  //@ bytes_to_dcells(cells_alloc, nat_of_int(index_range + DCHAIN_RESERVED));
  dchain_impl_init(chain_alloc->cells, size);
               ...
               double_chainp(dchain_allocate_fp(ch, ni, time), chain)); @*/
{
  ...
  bool ret = dchain_impl_allocate_new_index(pool->cells, out_index);
  //@ assert *index_out |-> ?ni;
  if (ret) {
    //@ extract_timestamp(timestamps, tstmps, ni);
    pool->timestamps[*out_index] = time;
              ...
              double_chainp(ch, chain)); @*/
{
  ...
  int ret = dchain_impl_rejuvenate_index(pool->cells, index);
                 ...
                 result == 0)))); @*/
{
  ...
  //@ int size = dchain_index_range_fp(ch);
  //@ assert times(timestamps, size, ?tmstmps);
  int has_ind = dchain_impl_get_oldest_index(pool->cells, out_index);
  //@ is_empty_def(ch, chi);
            ...
            dchain_allocated_fp(ch, index) ? result == 1 : result == 0; @*/
{
  ...
  /*@
    ...
    @*/
  bool result = dchain_impl_is_index_allocated(pool->cells, index);
  if (result) {
              ...
              (result == 0 &*& new_ch == ch); @*/
{
  ...
  @*/
  dchain_impl_free_index(pool->cells, index);
  /*@
