...
class PciPlugin(SimStatePlugin):
    ...
    def merge(self, others, merge_conditions, common_ancestor=None):
        if any(o._handle_read != self._handle_read or o._handle_write != self._handle_write for o in others):
            raise SymbexException("Cannot merge PciPlugin with different handlers")
        self._address = None
        self._flushed = False
        return True
    ...
    def handle_in(self, port, size):
        if port != 0xCFC:
            raise SymbexException("Unknown port to read from for port I/O")
        if not self._flushed:
            raise SymbexException("Attempt to read from PCI via I/O without having addressed and flushed first")
    ...
    def handle_out(self, port, data, size):
        if port == 0xCF8:
            ...
            if not self._flushed:
                raise SymbexException("Attempt to write to PCI via port I/O without having addressed and flushed first")
            self._handle_write(self.state, self._address, data)
        else:
            raise SymbexException("Unknown port to write to for port I/O")
