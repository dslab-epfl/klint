...
def spec_device_create_default(state, index):
    ...
    # Virt addr handling
    virt_addr = claripy.BVS("dev_virt_addr", state.sizes.ptr)
    state.memory.set_special_object(virt_addr, claripy.BVV(1, state.sizes.size_t), bar_size, device_reader, device_writer)
