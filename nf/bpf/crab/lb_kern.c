...
static __always_inline int handle_syn(struct xdp_md* ctx, struct ethhdr* ethh, struct iphdr* iph, struct tcphdr* tcph)
{
	...
	/* Fix TCP CSUM */
	tcph->check = 0;
	csum = bpf_csum_diff(0, 0, (__be32*) tcph, sizeof(struct tcphdr) + sizeof(struct redir_opt), 0);
