...
__attribute__((__always_inline__)) static inline void connection_table_lookup(struct real_definition** real, struct packet_description* pckt, void* lru_map)
{
...
__attribute__((__always_inline__)) static inline int process_packet(void* data, __u64 off, void* data_end, bool is_ipv6, struct xdp_md* xdp)
{
	...
	if (!dst) {
		...
		// REMOVED: __u32 cpu_num = bpf_get_smp_processor_id();
		void* lru_map = NULL; // REMOVED: bpf_map_lookup_elem(&lru_mapping, &cpu_num);
		if (!lru_map) {
		...
		if (pckt.flow.proto == IPPROTO_TCP && !(pckt.flags & F_SYN_SET)) {
			...
			if (tcp_hdr_opt_lookup(data, data_end, is_ipv6, &dst, &pckt, vip_info->flags & F_LRU_BYPASS, lru_map) == FURTHER_PROCESSING) {
				routing_stats->v1 += 1;
			} else {
				routing_stats->v2 += 1;
			}
