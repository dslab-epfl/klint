...
bool is_packet_in_whitelist(struct net_ipv4_header* ipv4_packet)
{
	...
	struct ipv4_address_key key_dst = {true, dst_ip};
...
bool nf_init(device_t devices_count)
{
	...
	if (!os_config_get_device("check device", devices_count, &check_device) || !os_config_get_device("command device", devices_count, &command_device) ||
	    !os_config_get_device("send device", devices_count, &send_device) || send_device == command_device || send_device == check_device) {
		return false;
	}
